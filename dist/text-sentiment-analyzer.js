...
function analyzeEmojis(text) {
    ...
    for (const [sentiment, pattern] of Object.entries(SENTIMENT_PATTERNS)) {
        ...
        if (matchCount > 0) {
            scores.set(sentiment, matchCount * pattern.weight * 0.8);
        }
    }
    return scores;
}
...
function analyzeKeywords(text) {
    ...
    for (const [sentiment, pattern] of Object.entries(SENTIMENT_PATTERNS)) {
        ...
        if (matchScore > 0) {
            ...
            scores.set(sentiment, matchScore + bonus);
        }
    }
    return scores;
}
 ...
 */
function applyContext(scores, context) {
 ...
 */
function calculateConfidence(primaryScore, scores, text) {
 ...
 */
export function detectSentiment(text, context) {
    // Collect evidence
    const evidence = [];
    ...
    for (const sentiment of Object.keys(SENTIMENT_PATTERNS)) {
        const total = (keywordScores.get(sentiment) || 0) +
    ...
    if (keywordScores.get(primarySentiment)) {
        ...
        const matchedKeywords = pattern.keywords.filter(kw => text.toLowerCase().includes(kw));
 ...
 */
export function detectSentimentsBatch(messages) {
    const results = [];
 ...
 */
export function getSentimentTypes() {
    return Object.keys(SENTIMENT_PATTERNS);
}
/**
 * Get sentiment pattern data
 */
export function getSentimentPattern(sentiment) {
    return SENTIMENT_PATTERNS[sentiment];
}
/**
 * Check if sentiment is positive
 */
export function isPositiveSentiment(sentiment) {
 ...
 */
export function isHighArousal(sentiment) {
    const pattern = SENTIMENT_PATTERNS[sentiment];
...
export function getSentimentIntensity(sentiment) {
    ...
    if (!pattern)
        return 'medium';
    if (pattern.vad.arousal > 0.7)
        return 'high';
    if (pattern.vad.arousal > 0.4)
        return 'medium';
    return 'low';
}
